    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-300">
      ...
      <header className="relative pt-32 pb-16 md:pt-40 md:pb-24 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900 dark:text-white">
            Simple, <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-sky-400 dark:to-indigo-400">Predictable</span> Pricing
          </h1>
      ...
      <main className="max-w-7xl mx-auto px-6 pb-24">
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative bg-white dark:bg-slate-900 rounded-[2rem] p-8 transition-all duration-300 flex flex-col hover:-translate-y-2 ${
                plan.isPopular
                  ? 'border-2 border-blue-500 dark:border-sky-500 shadow-2xl shadow-blue-500/10 dark:shadow-sky-500/20 scale-105 z-10'
                  : 'border border-slate-200/60 dark:border-slate-800 shadow-lg hover:shadow-xl dark:shadow-slate-950/50'
              }`}
            >
              {plan.isPopular && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1.5 bg-blue-500 dark:bg-sky-500 text-white text-sm font-bold rounded-full shadow-md">
                  Most Popular
                </div>
              )}
              ...
              <div className="mb-8">
                ...
                <div className="flex items-end gap-1 mb-4">
                  ...
                  {plan.priceMonthly !== 'Custom' && <span className="text-slate-500 dark:text-slate-400 font-medium mb-1">/mo</span>}
                </div>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-16 text-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 p-8 md:p-12 max-w-3xl mx-auto shadow-sm"
        >
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">All Plans Include</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
            {[
              ...
              <div key={i} className="flex items-center gap-2 text-slate-700 dark:text-slate-300 font-medium">
                <AppIcon name="CheckCircle2" className="w-5 h-5 text-emerald-500 dark:text-emerald-400 flex-shrink-0" />
                {item}
              </div>
            ))}
          </div>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-12 text-center bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 dark:from-slate-900 dark:to-slate-950 rounded-3xl p-12 md:p-16 border border-slate-800"
        >
