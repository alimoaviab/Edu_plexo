...
export function StudentDetailsModal({ isOpen, studentId, onClose }: StudentDetailsModalProps) {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="xl"
      className="!p-0 overflow-hidden bg-white/95 backdrop-blur-md rounded-3xl"
    >
      {/* Modal Header */}
      <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <div className="flex items-center gap-4">
          <div className="h-12 w-12 rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white flex items-center justify-center font-bold text-lg shadow-md shadow-blue-500/25">
            {student ? `${student.first_name[0]}${student.last_name ? student.last_name[0] : ""}`.toUpperCase() : <AppIcon name="User" size={24} />}
          </div>
          <div>
        ...
        <button
          onClick={onClose}
          className="h-9 w-9 rounded-xl border border-slate-200/80 bg-white text-slate-400 hover:text-slate-700 hover:bg-slate-50 flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-sm z-10"
        >
