...
export function TestMarksEntryPage({ testId: testIdProp, role = "TEACHER" }: { testId?: string; role?: "ADMIN" | "TEACHER" }) {
  ...
  useEffect(() => {
    ...
    void runTest(async () => {
      const result = await serviceRequest<TestDetails>(`/api/tests/${testId}`);
      if (!result.ok) throw new Error(result.error.message || "Failed to load test details");
      return result.data;
    });
  }, [runTest, testId]);

  useEffect(() => {
    if (!testState.data?.class_id) return;

    void runStudents(async () => {
      const result = await serviceRequest<Student[]>(`/api/students?class_id=${testState.data?.class_id}&status=active`);
      if (!result.ok) throw new Error(result.error.message || "Failed to load students");
      return result.data;
    });
  }, [runStudents, testState.data?.class_id]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const result = await serviceRequest<any[]>(`/api/tests/${testId}/results`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await serviceRequest(`/api/tests/${testId}/results`, {
        method: "POST",
      ...
      showToast("Testination marks synchronized successfully.", "success");
    } catch (err: any) {
  ...
  if (testState.status === "error") {
    return <DataState variant="error" title="Workspace Load Failed" message={testState.error} />;
  }
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <Badge variant="secondary" className="bg-slate-50 text-slate-600 border-slate-100 font-black uppercase text-[9px]">{test.class_name}</Badge>
                  ...
                  <tr key={student._id} className={`transition-colors ${isAbsent ? "bg-red-50/20" : "hover:bg-slate-50/50"}`}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className="text-[10px] font-bold text-slate-300">/ {test.max_marks}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <Input 
