 ...
 * Exam API client.
 *
 * Architecture: one exam → many subjects. Create posts a single record
 * with a `subjects[]` array; the server stores it as one row and the
 * list page renders one card. The legacy "bulk fan-out" helper that
 * created N rows for N subjects has been removed — the new architecture
 * makes it unnecessary and keeping it would re-introduce the duplicate
 * cards bug.
 */
...
export function createExam(input: ExamFormInput) {
  return serviceRequest<ExamRow>("/api/exams", {
    method: "POST",
    body: JSON.stringify({ ...input, type: input.type || "exam" }),
  });
}
