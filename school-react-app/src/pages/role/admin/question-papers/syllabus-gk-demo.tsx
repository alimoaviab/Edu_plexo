...
import { PTB_CLASS1_GENERAL_KNOWLEDGE, getAllChapters } from "@/data/syllabus/ptb-class1-general-knowledge";

/**
 * General Knowledge Syllabus Demo Page
 * 
 * Demonstrates the Chapter Selector with PTB Class 1 General Knowledge (Urdu).
 */
...
export function SyllabusGKDemoPage() {
  ...
  return (
    <SchoolShell eyebrow="Question Papers" title="General Knowledge - Chapter Selection">
      <div className="space-y-6">
        ...
        <div className="p-4 bg-blue-50/50 border border-blue-100/50 rounded-xl">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[11px] font-bold text-blue-900 mb-1">Urdu Chapter Support</p>
              <p className="text-[10px] text-blue-700 leading-relaxed">
                This syllabus includes Urdu chapter names with proper RTL (right-to-left) text rendering. 
                All 14 chapters from PTB Class 1 General Knowledge are included.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
              <ChapterSelector
                units={PTB_CLASS1_GENERAL_KNOWLEDGE}
                selectedChapters={selectedChapters}
                onSelectionChange={setSelectedChapters}
                title="PTB Class 1 General Knowledge Syllabus"
                subtitle="Select chapters to include in the question paper"
              />
            </div>
          </div>

          {/* Right: Selected Chapters Summary */}
                ...
                <div className="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar">
                  {selectedChapterDetails.map((ch) => (
                    ...
                    >
                      ...
                      <div className="text-[10px] text-slate-500 pr-6" dir="rtl">{ch.titleUrdu}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-[11px]">
                  ...
                  <span className="font-bold text-slate-900">{PTB_CLASS1_GENERAL_KNOWLEDGE.length}</span>
                </div>
                <div className="flex items-center justify-between text-[11px]">
